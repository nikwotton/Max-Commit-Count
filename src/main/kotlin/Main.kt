...
suspend fun main() {
    ...
    val numCommits: Int = 1
    try {
        val client = HttpClient()
        println("Got to 1")
        val foo = client.get<JsonObject>(inputs.repoUrl) {
...
//        getInput("maxCommits").ifEmpty { throw IllegalArgumentException("Don't forget to define maxCommits") }
    val inputNumber =
        2// inputString.toIntOrNull() ?: throw IllegalArgumentException("$inputString is not a valid number")
    val token = ""
...
//    val repoUrl = "https://api.github.com/repos/$repo/pulls/$prNumber"
    return Inputs(maxCommits = inputNumber, token = token, repoUrl = "https://www.google.com/")
}
